...
class PaperTrader:
    def __init__(self, size_percentage: float = 0.05, max_trade_usd: float = 5.0,
        ...
        if live_mode:
            ...
            if not pk:
                raise ValueError("POLYMARKET_PRIVATE_KEY not set — cannot start live mode.")
...
class TelegramController:
    ...
    def _cmd_confirmgolive(self):
        ...
        if not os.getenv("POLYMARKET_PRIVATE_KEY", "").strip():
            self.send("❌ POLYMARKET_PRIVATE_KEY not set in .env — cannot go live.")
            return
...
def run_promote_to_live():
    ...
    # Confirm live mode
    print("\n⚠️  WARNING: LIVE MODE will place REAL orders on Polymarket using your POLYMARKET_PRIVATE_KEY.")
    print("   Make sure your .env file has POLYMARKET_PRIVATE_KEY set correctly.")
    ...
    try:
        ...
        print("    Set POLYMARKET_PRIVATE_KEY in your .env file and try again.")
